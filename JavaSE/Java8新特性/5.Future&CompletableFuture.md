...
> CompletableFuture能够将回调放到与任务放到不同的线程中执行，也能将回调作为继续执行的同步函数，在与任务相同的线程中执行。它避免了传统回调最大的问题，那就是能够将控制流分离到不同的事件处理器中。
>
...
* **thenCompose 与 thenComposeAsync ：** 在异步操作完成的时候对异步操作的结果进行一些操作，并且仍然返回CompletableFuture类型，相当于flatMap。不同之处是他们底层使用了不同的线程池。
...
1. 仔细学习并区别thenApply与thenCompose的异同，并举例说明。

2. 完成下面需求：

   * 有三个用户：张三，身份证号为1，李四，身份证号为1，王五， 身份证号为1。
   * 有个用户服务可以通过身份证号来查到对应的用户。
   * 张三有一张编号为123的农行卡，金额为100元，李四有一张编号为234的建行卡，金额为500元。
   * 有个银行服务能够通过用户来查询该用户的卡类型和余额。
